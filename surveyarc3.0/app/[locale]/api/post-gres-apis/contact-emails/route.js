...
// POST /api/post-gres-apis/contact-emails
export async function POST(req) {
  try {
    const body = await req.json();
    const payload = await encryptPayload(body);
    
    const res = await fetch(`${BASE}/contact-emails/`, {
      method: "POST",
      headers: { 
...
// GET /api/post-gres-apis/contact-emails?contact_id=xxx (optional)
export async function GET(req) {
  ...
  try {
    const url = contact_id 
      ? `${BASE}/contact-emails?contact_id=${encodeURIComponent(contact_id)}`
      : `${BASE}/contact-emails`;
