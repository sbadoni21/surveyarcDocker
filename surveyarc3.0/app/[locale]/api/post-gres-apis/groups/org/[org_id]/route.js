// app/en/api/post-gres-apis/groups/org/[org_id]/route.js
...
// GET /en/api/post-gres-apis/groups/org/[org_id]
export async function GET(req, { params }) {
  try {
    const { org_id } = await params; // ❌ no await
    const authHeader = req.headers.get("authorization");
    const cookieHeader = req.headers.get("cookie");
    const userHeader = req.headers.get("x-user-id");

    const res = await fetch(`${BASE}/groups/org/${encodeURIComponent(org_id)}`, {
      headers: {
        ...(ENC ? { "x-encrypted": "1" } : {}),
    ...
    console.error("[groups org GET] error:", error);
    return NextResponse.json(
