// app/api/post-gres-apis/tickets/[ticket_id]/group/route.js
...
// POST assign/clear group_id
export async function POST(req, { params }) {
  const { ticket_id } = await params;
  try {
    const raw = await req.json(); // { group_id?: string | null }
    const payload = ENC ? await encryptPayload(raw) : raw;
    const res = await fetch(`${BASE}/tickets/${encodeURIComponent(ticket_id)}/group`, {
      method: "POST",
