...
// POST /api/post-gres-apis/contact-socials
export async function POST(req) {
  try {
    const body = await req.json();
    const payload = await encryptPayload(body);
    
    const res = await fetch(`${BASE}/contact-socials/`, {
      method: "POST",
      headers: { 
...
// GET /api/post-gres-apis/contact-socials?contact_id=xxx (optional)
export async function GET(req) {
  ...
  try {
    const url = contact_id 
      ? `${BASE}/contact-socials?contact_id=${encodeURIComponent(contact_id)}`
      : `${BASE}/contact-socials`;
