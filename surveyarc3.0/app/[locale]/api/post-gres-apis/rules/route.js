...
async function forceDecryptResponse(res) {
  ...
  // array or object envelope(s)
  if (Array.isArray(parsed.json)) {
    const out = await Promise.all(parsed.json.map(async (item) => (looksEnvelope(item) ? await decryptGetResponse(item) : item)));
...
export async function GET(req) {
  ...
  try {
    const res = await fetch(`${BASE}/rules/${encodeURIComponent(survey_id)}`, { signal: AbortSignal.timeout(30000) });
