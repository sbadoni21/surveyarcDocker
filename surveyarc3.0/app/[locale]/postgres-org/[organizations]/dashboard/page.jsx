...
function OrgSwitcher({ currentOrgId }) {
  ...
  const onChange = (e) => {
    ...
    router.push(`/en/postgres-org/${selected}/dashboard`);
  };
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const checkExpiry = async () => {
      ...
      const endDate = organisation.subscription.endDate
        ? new Date(organisation.subscription.endDate)
        : null;
      ...
      setCookie("subscriptionEnd", formattedEndDate.toISOString(), {
        path: "/", // accessible everywhere
        maxAge: 60 * 60 * 24 * 30, // 30 days
      });
      const now = new Date();

      if (now > formattedEndDate) {
        try {
  ...
  useEffect(() => {
    if (!organisation?.org_id || !organisation?.subscription?.endDate) return;

    const rawEndDate = organisation.subscription.endDate;

    const endDate = rawEndDate?.seconds
      ? new Date(rawEndDate.seconds * 1000) // Firestore timestamp
      : new Date(rawEndDate); // ISO string or Date
    ...
    if (diffDays > 0 && diffDays <= 30) {
      ...
      const todayKey = `${organisation.org_id}-${today.toDateString()}`;

      if (lastMailSent !== todayKey) {
        fetch("/api/post-gres-apis/send-expiry-mail", {
          method: "POST",
  ...
  function getEndDate(rawEndDate) {
    if (!rawEndDate) return null;

    if (rawEndDate && rawEndDate.seconds) {
      return new Date(rawEndDate.seconds * 1000);
    }
    ...
    <div className="min-h-screen">
      <div className="p-6 sm:p-10">
        ...
        {organisation?.subscription?.endDate &&
          (() => {
            const endDate = getEndDate(organisation.subscription.endDate);
        ...
        >
          ...
          {isExpired ? (
            ...
            <span className="font-medium text-black dark:text-white">
              Plan Expires – {organisation?.subscription?.formattedEndDate}
            </span>
          )}
        </div>
            ...
            {organisation?.subscription?.quota &&
              organisation?.subscription?.currentUsage && (
                <QuotaUsageGrid
                  ...
                  team={organisation.team_members}
                />
              )}
