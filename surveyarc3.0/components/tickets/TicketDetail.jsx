...
const pickName = (u) =>
  u?.display_name ?? u?.displayName ?? u?.full_name ?? u?.name ?? u?.email ?? u?.user_id ?? u?.userId ?? u?.uid;
...
export default function TicketDetail({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const catId = draft?.category || ticket?.categoryId || ticket?.category;
      if (catId) {
    ...
    return () => { mounted = false; };
  }, [effectiveOrgId, listCategories, listProducts, listSubcategories, draft?.category, ticket?.categoryId, ticket?.category]);

  const catName = useMemo(() => {
    const id = draft?.category ?? ticket?.categoryId ?? ticket?.category;
    const rec = (cats || []).find((x) => (x.category_id ?? x.categoryId) === id || x.id === id);
  ...
  const subName = useMemo(() => {
    ...
    const rec = (subs || []).find((x) => (x.subcategory_id ?? x.subcategoryId) === id || x.id === id);
  ...
  const prodName = useMemo(() => {
    ...
    const rec = (prods || []).find((x) => (x.product_id ?? x.productId) === id || x.id === id);
  ...
  const tagChips = useMemo(() => {
    const explicit = (ticket.tags || []).map((t) => ({
    ...
    return (Array.isArray(tagIds) ? tagIds : []).map((idLike) => {
      const id = typeof idLike === "string" ? idLike : (idLike.tag_id ?? idLike.tagId ?? idLike.id);
              ...
              <Box flex={1}>
                <Stack direction="row" spacing={1.5} alignItems="center" mb={1}>
                  ...
                  <Chip 
                    ...
                    size="small"
                    sx={{ 
                      bgcolor: alpha(STATUS_COLORS[draft.status] || STATUS_COLORS.open, 0.1),
                      color: STATUS_COLORS[draft.status] || STATUS_COLORS.open,
                      fontWeight: 600,
                      borderColor: STATUS_COLORS[draft.status] || STATUS_COLORS.open
                    }}
                    variant="outlined"
                  />
                  <Chip 
                    ...
                    sx={{ 
                      bgcolor: alpha(PRIORITY_COLORS[draft.priority] || PRIORITY_COLORS.normal, 0.1),
                      color: PRIORITY_COLORS[draft.priority] || PRIORITY_COLORS.normal,
                      fontWeight: 600
                    }}
                  />
                </Stack>
                ...
                {edit ? (
                  <>
                    ...
                    <Tooltip title="Cancel">
                      <IconButton 
                        onClick={() => { setDraft(ticket); setEdit(false); }}
                        disabled={saving}
                      >
                        <CloseIcon />
                      </IconButton>
          ...
          <Box sx={{ p: 3 }}>
            <Stack spacing={3}>
              ...
              <Card variant="outlined" sx={{ borderRadius: 2 }}>
                <CardContent>
                  <Typography variant="h6" fontWeight={600} mb={2.5} display="flex" alignItems="center" gap={1}>
                  ...
                  <Stack spacing={2.5}>
                            ...
                            <GroupSelect
                              ...
                              onChange={(v) => {
                                ...
                                setDraft((d) => ({
                                  ...d,
                                  groupId: v ?? null,
                                  assigneeId: d.groupId !== v ? null : d.assigneeId,
                                }));
                              }}
                            />
                          </Box>
                    ...
                    <Stack direction="row" spacing={3} flexWrap="wrap">
                      <PropertyField
                        ...
                        edit={edit}
                        onChange={(v) => setDraft((d) => ({ ...d, slaId: v }))}
                        icon={<AccessTimeIcon fontSize="small" />}
                      />
                      
                      <PropertyField
                        label="Due Date"
                        value={edit ? (draft.dueAt || "") : (ticket.dueAt ? new Date(ticket.dueAt).toLocaleString() : "—")}
                        edit={edit}
                        onChange={(v) => setDraft((d) => ({ ...d, dueAt: v }))}
                ...
                <CardContent>
                  ...
                  {edit ? (
                    ...
                    <Typography 
                      ...
                      sx={{ 
                        whiteSpace: "pre-wrap",
                        color: ticket.description ? "text.primary" : "text.secondary",
                          ...
                          {draft.teamId && (
                            <>
                              <Button 
                                size="small" 
                                variant="contained"
                                onClick={() => onAssignTeam?.(ticket.ticketId, draft.teamId)}
                              >
                                Assign
                              </Button>
                              <Button 
                        ...
                        <Stack direction="row" spacing={1} alignItems="center">
                          <Box flex={1}>
                            <AssigneeSelect
                              ...
                              groupId={draft.groupId}
                              value={draft.agentId}
                              onChange={(v) => setDraft((d) => ({ ...d, agentId: v }))}
                              label=""
                          ...
                          {draft.agentId && (
                            <>
                              <Button 
                                size="small" 
                                variant="contained"
                                onClick={() => onAssignAgent?.(ticket.ticketId, draft.agentId)}
                              >
                                Assign
                              </Button>
                              <Button 
                        ...
                        <Stack direction="row" spacing={1} flexWrap="wrap" gap={1}>
                          {collabs.length ? (
                            collabs.map((c) => (
                              <Chip
                                key={c.user_id}
                                label={`${collabDisplays[c.user_id] || c.user_id}${c.role ? ` • ${c.role}` : ""}`}
                                onDelete={async () => {
                ...
                <CardContent>
                  ...
                  >
                    ...
                    <Stack direction="row" spacing={1} alignItems="center">
                      ...
                      <IconButton size="small">
                        {showWorklogs ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                      </IconButton>
                    </Stack>
                  </Stack>
                    ...
                    <Box mt={2.5}>
                      {logs?.length ? (
                            ...
                            >
                              <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                                ...
                                <Typography variant="caption" color="text.secondary" sx={{ whiteSpace: "nowrap", ml: 2 }}>
                                  {new Date(wl.created_at).toLocaleDateString()}<br />
                                  {new Date(wl.created_at).toLocaleTimeString()}
                                </Typography>
                              </Stack>
                            </Paper>
                          ))}
                        ...
                        <Alert severity="info" sx={{ borderRadius: 2 }}>
                          No work logged yet. Click "Log Work" to add your first entry.
                        </Alert>
                      )}
                    </Box>
                  </Collapse>
...
function PropertyField({ label, value, edit, type = "text", options, onChange, icon, placeholder }) {
  return (
