...
import { Plus, Trash2, ChevronDown, ChevronRight, GitBranch, Play, Filter, X, Save, Sparkles } from 'lucide-react';
...
const csvToArray = (s) => (s || '').split(',').map((t) => t.trim()).filter(Boolean);
...
/* =============================================
   MERGED SCREEN
============================================= */
export default function SurveyFlowView({ surveyId: surveyIdProp, blocks = [], questions = [] }) {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        const shouldFetch = !fetchedSurveys.current.has(surveyId) && !fetchInFlight.current;
    ...
    if (surveyId) run(); else setLoadingRules(false);
    return () => { active = false; };
  ...
  const blocksResolved = useMemo(() => {
    ...
    if (stray.length) list.push({ blockId: '__unassigned__', name: 'Unassigned', _questions: stray });
    return list;
  ...
  const qToBlockId = useMemo(() => {
    const m = new Map();
    blocksResolved.forEach((b) => b._questions.forEach((q) => m.set(q.questionId, b.blockId)));
    return m;
  }, [blocksResolved]);
  ...
  const blockLayouts = useMemo(() => {
    ...
    for (const b of blocksResolved) {
      ...
      out.push({ blockId: b.blockId, name: b.name || 'Block', x: cursorX, y: 0, width, height });
      cursorX += width + 40;
    }
    return out;
  }, [blocksResolved]);
  ...
  const blockNodes = useMemo(() => blockLayouts.map((l) => ({
    id: `block-${l.blockId}`,
    type: 'group',
    ...
    style: {
      ...
      border: l.blockId === '__unassigned__' ? '2px dashed #EF4444' : '2px solid #3b3b45',
  ...
  const blockLabelNodes = useMemo(() => blockLayouts.map((l) => ({
    ...
    style: {
      width: 'max-content',
      padding: '4px 12px',
      border: l.blockId === '__unassigned__' ? '1px dashed #EF4444' : '1px solid #3b3b45',
      borderRadius: 999,
      background: '#000',
    ...
    blocksResolved.forEach((b) => {
      ...
      b._questions.forEach((q, i) => {
        nodes.push({
          ...
          extent: 'parent',
          position: { x: BLOCK_PADDING + i * X_GAP, y: BLOCK_PADDING + BLOCK_HEADER_H },
          style: {
            borderRadius: 12,
            padding: 14,
  ...
  const ruleGroups = useMemo(() => {
    const g = {};
    (rules || []).forEach((r) => {
      const primary = (r.conditions || []).find((c) => !!c.questionId)?.questionId || '__ungrouped__';
  ...
  const { logicNodes, messageNodes } = useMemo(() => {
    ...
    const orderPrimaries = [
      ...
      ...Object.keys(ruleGroups).filter((k) => k !== '__ungrouped__' && !globalQ.some((q) => q.questionId === k)),
  ...
  const edges = useMemo(() => {
    ...
    (rules || []).forEach((rule) => {
      ...
      (rule.actions || []).forEach((a, idx) => {
        ...
        const primaryQ = (rule.conditions || []).find((c) => !!c.questionId)?.questionId || null;
        const res = resolveActionOrSkipTargetId({ action: a, primaryQuestionId: primaryQ, blockEntryMap, qToBlockId, globalQ, blocks: blocksResolved });
        if (res?.targetId) {
          ...
          arr.push({
            ...
            type: 'smoothstep',
            animated: true,
            style: { stroke, strokeWidth: isSkip ? 3 : 3, strokeDasharray: isSkip ? '6 3' : undefined },
            markerEnd: { type: MarkerType.ArrowClosed, color: stroke },
            label: actionEdgeLabel(a, res.targetId, blocksResolved, res.meta),
            labelStyle: { fontSize: 11, fill: '#E5E7EB', fontWeight: 500 },
            labelBgPadding: [6, 4],
            labelBgBorderRadius: 8,
            labelBgStyle: { fill: isGoto ? 'rgba(6,78,59,.95)' : isSkip ? 'rgba(120,53,15,.95)' : 'rgba(127,29,29,.95)', stroke },
          });
        }
      });
    });
  ...
  const normalizeActions = (actions) => (actions || []).map((act) => {
    const base = { type: act.type };
    if (act.type === 'goto_block') base.blockId = act.blockId || '';
    if (act.type === 'goto_block_question') { base.targetBlockId = act.targetBlockId || ''; base.targetQuestionId = act.targetQuestionId || ''; }
  ...
  function openTicketFormForAction(act, index) {
    ...
    setTicketInitial({
      subject: (act.subjectTemplate || '').trim() || `Ticket for rule ${editingRule?.name || editingRule?.ruleId || ''}`,
  ...
  return (
    <div className="grid grid-cols-1 2xl:grid-cols-[1fr_320px] gap-5 w-full h-[88vh]">
      <div className="w-full h-full dark:bg-[#0D0D0F] p-4 rounded-xl border-2 border-neutral-800 shadow-2xl overflow-hidden">
        ...
        <ReactFlow
          nodes={[...blockNodes, ...blockLabelNodes, ...questionNodes, ...logicNodes, ...messageNodes, endNode]}
          edges={edges}
          fitView
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3 p-4 bg-indigo-50 border border-indigo-200 rounded-xl">
              ...
              <div className="text-sm text-indigo-900">
                ...
                <p className="text-indigo-700">Click a <span className="font-semibold">question node</span> to create a new branch. Click a <span className="font-semibold">logic node</span> to edit existing rules.</p>
              </div>
            </div>
          </div>
        ) : (
...
============================================= */
function RuleEditor({
  rule, setRule, onSave, onDelete, saving,
  blocksResolved, questionsById, availableTags,
  openTicketFormForAction, ticketOpen, setTicketOpen,
  ticketInitial, setTicketInitial, ticketActIndex, setTicketActIndex,
  orgId, currentUserId,
}) {
  const blockOrder = useMemo(() => blocksResolved.map((b) => b.blockId), [blocksResolved]);
  const blocksById = useMemo(() => Object.fromEntries(blocksResolved.map((b) => [b.blockId, b])), [blocksResolved]);
  ...
  const getOperatorsForType = (type) => {
    ...
    if (type === 'number') base.push({ label: 'Greater Than', value: 'greater_than' }, { label: 'Less Than', value: 'less_than' });
    if (type === 'nps') {
      return [
  ...
  return (
    <div className="space-y-5 text-sm">
      <div className="flex items-center justify-between pb-4 border-b-2 border-gray-200">
        <h3 className="font-bold text-xl text-slate-900">{rule.ruleId ? 'Edit Branch' : 'Create Branch'}</h3>
        <button 
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Branch Name</label>
          <input 
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Applies to Block</label>
            <select 
              ...
              onChange={(e) => {
                ...
                setRule((prev) => ({
                  ...prev,
                  blockId: newBlockId,
                  conditions: (prev.conditions || []).map((c) => ({ ...c, questionId: '', value: '' })),
                  actions: (prev.actions || []).map((a) => ({ ...a, questionId: '', questionIds: [] })),
                }));
              }}>
              {blockOrder.map((bid) => (
                <option key={bid} value={bid}>{blocksById[bid]?.name || bid}</option>
              ))}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Priority</label>
            <input 
              type="number" 
              className="w-full border-2 border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 p-3 rounded-lg text-sm transition-all outline-none" 
              value={rule.priority}
        ...
        <label className="inline-flex items-center gap-2.5 text-sm cursor-pointer group">
          ...
          <span className="font-medium text-slate-700 group-hover:text-slate-900">Enabled</span>
        </label>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full font-medium">IF</span>
          </div>
          
          {rule.conditions.map((cond, index) => {
            const allQs = questionsByBlock[rule.blockId] || [];
            const selectedQ = allQs.find((q) => q.questionId === cond.questionId) || null;

            return (
              <div key={index} className="p-4 rounded-xl border-2 border-gray-200 bg-gray-50 space-y-3 hover:border-indigo-300 transition-colors">
                ...
                <div className="grid gap-2">
                  <select 
                    ...
                    {allQs.map((q) => (
                      <option key={q.questionId} value={q.questionId}>{q.label}</option>
                    ))}
                  </select>
                  ...
                  {selectedQ && (
                    <>
                      ...
                      {selectedQ?.type === 'nps' ? (
                        ...
                        <input 
                          type={selectedQ.type === 'number' ? 'number' : 'text'} 
                          className="w-full border-2 border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 p-2.5 rounded-lg text-sm transition-all outline-none" 
                ...
                <div className="flex gap-2 pt-2">
                  {index === rule.conditions.length - 1 && (
                    <button 
                      ...
                      onClick={() => setRule((r) => ({ ...r, conditions: [...r.conditions, { ...emptyCondition }] }))}>
                  ...
                  {rule.conditions.length > 1 && (
                    <button 
                      ...
                      onClick={() => setRule((r) => ({ ...r, conditions: r.conditions.filter((_, i) => i !== index) }))}>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-medium">THEN</span>
          </div>

          {rule.actions.map((act, index) => (
            <div key={index} className="p-4 rounded-xl border-2 border-gray-200 bg-gray-50 space-y-3 hover:border-emerald-300 transition-colors">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs font-bold text-gray-500">#{index + 1}</span>
              </div>
              ...
              {act.type === 'goto_block_question' && (
                <div className="grid gap-2">
                  ...
                  <select 
                    ...
                    <option value="">Select Question</option>
                    {(blocksResolved.find((b) => b.blockId === act.targetBlockId)?._questions || []).map((q) => (
                      <option key={q.questionId} value={q.questionId}>{q.label}</option>
                    ))}
                  </select>
                </div>
              ...
              {act.type === 'goto_question' && (
                <select 
                  ...
                  <option value="">Select Question</option>
                  {(blocksResolved.find((b) => b.blockId === rule.blockId)?._questions || []).map((q) => (
                    <option key={q.questionId} value={q.questionId}>{q.label}</option>
                  ))}
                </select>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold bg-red-50 hover:bg-red-100 text-red-600 border-2 border-red-200 rounded-lg transition-colors"
                  onClick={() => setRule((r) => ({ ...r, actions: r.actions.filter((_, i) => i !== index) }))}>
                  <Trash2 className="w-3.5 h-3.5" />
                  Remove
                </button>
              </div>
          ...
          <button 
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors" 
            onClick={() => setRule((r) => ({ ...r, actions: [...r.actions, { ...emptyAction }] }))}>
        ...
        <div className="flex gap-3 pt-4 border-t-2 border-gray-200">
          <button 
            ...
            {saving ? 'Saving...' : (rule.ruleId ? 'Update Branch' : 'Save Branch')}
          </button>
          {rule.ruleId && (
            <button 
...
/* =============================================
   HELPERS
============================================= */
function valuePreview(v) { if (v == null) return 'null'; if (Array.isArray(v)) return v.map(safeStr).join(' | '); return safeStr(v); }
function safeStr(v) { try { if (typeof v === 'object') return JSON.stringify(v); return String(v); } catch { return String(v); } }
function conditionLabel(cond) { const op = cond?.operator || ''; return `${op}: ${valuePreview(cond?.value)}`; }
function actionPreview(a) {
  switch (a?.type) {
    ...
    case 'raise_ticket': return `raise ticket`;
...
function actionEdgeLabel(a, toId, blocks = [], meta = {}) {
  ...
  switch (a.type) {
    ...
    case 'goto_block': { const blk = blocks.find((b) => b._questions?.[0]?.questionId === toId) || null; const nm = blk?.name || a.blockId; return `→ Block: ${nm}`; }
    case 'goto_block_question': return `→ Q: ${toId}`;
    case 'skip_question': return '⤼ Skip question';
    case 'skip_block': { const nextName = meta?.nextBlockName ? ` (${meta.nextBlockName})` : ''; return `⤼ Skip block${nextName}`; }
    case 'end':
    case 'skip_end': return 'End Survey';
    case 'show_message': return 'Show Message';
...
function resolveActionOrSkipTargetId({ action, primaryQuestionId, blockEntryMap, qToBlockId, globalQ, blocks }) {
  if (!action) return null;
  const getNextQuestionGlobal = (qid) => { const idx = globalQ.findIndex((q) => q.questionId === qid); return (idx >= 0 ? globalQ[idx + 1] : null)?.questionId || null; };
  const getNextQuestionInSameBlock = (qid) => { const blockId = qToBlockId.get(qid); if (!blockId) return null; const same = globalQ.filter((q) => qToBlockId.get(q.questionId) === blockId); const i = same.findIndex((q) => q.questionId === qid); return (i >= 0 ? same[i + 1] : null)?.questionId || null; };
  const getNextBlockEntry = (qid) => {
    const blockId = qToBlockId.get(qid); if (!blockId) return null;
    const seq = []; const seen = new Set();
    globalQ.forEach((q) => { const b = qToBlockId.get(q.questionId); if (b && !seen.has(b)) { seen.add(b); seq.push(b); } });
    const idx = seq.indexOf(blockId); const nextBlockId = idx >= 0 ? seq[idx + 1] : null; if (!nextBlockId) return null;
  ...
  switch (action.type) {
    case 'goto_block': return { targetId: action.blockId ? (blockEntryMap.get(action.blockId) || null) : null };
    case 'goto_question': return { targetId: action.questionId || null };
    case 'goto_block_question': return { targetId: action.targetQuestionId || null };
    case 'skip_question': { const nextInBlock = primaryQuestionId ? getNextQuestionInSameBlock(primaryQuestionId) : null; const next = nextInBlock || (primaryQuestionId ? getNextQuestionGlobal(primaryQuestionId) : null); return { targetId: next || null }; }
    case 'skip_block': { const res = primaryQuestionId ? getNextBlockEntry(primaryQuestionId) : null; if (!res) return null; return { targetId: res.entry || null, meta: { nextBlockName: res.nextBlockName } }; }
    default: return null;
  }
}
function messageNodeStyle() {
  return {
    ...
    borderRadius: 12,
    padding: 10,
    background: 'linear-gradient(135deg, rgba(15,118,110,.25) 0%, rgba(15,118,110,.15) 100%)',
    color: '#000',
  ...
  return (
    <div className="hidden md:flex items-center gap-4 text-xs bg-gray-900 px-4 py-2.5 rounded-xl border border-gray-700">
      <span className="inline-flex items-center gap-2 text-gray-300">
        <i className="w-3 h-3 inline-block rounded-sm border-2 border-gray-500" /> Block
      </span>
      <span className="inline-flex items-center gap-2 text-gray-300">
        <i className="w-3 h-3 inline-block rounded-sm border-2 border-dashed border-red-500" /> Unassigned
      </span>
      <span className="inline-flex items-center gap-2 text-gray-300">
        <i className="w-3 h-3 inline-block rounded-sm bg-indigo-600" /> Condition
      </span>
      <span className="inline-flex items-center gap-2 text-gray-300">
        <i className="w-3 h-3 inline-block rounded-sm bg-emerald-600" /> Goto / End
      </span>
      <span className="inline-flex items-center gap-2 text-gray-300">
